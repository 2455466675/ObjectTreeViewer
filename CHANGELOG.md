...
### Added
- 刷新数据时按位置保留树的展开/收起状态：为节点引入与构建顺序、Id 无关的稳定路径（`StablePath`），刷新后沿原展开分支做懒加载增量恢复。列表元素采用位置索引匹配，原位置不存在时自动跳过（中间插入/删除按位置语义处理）。

## [1.4.0]

### Changed
- 路径解析不再随机采用第一个同名简单类型；发现多个候选时在错误日志中列出完整类型名与程序集。
- 新增 `命名空间::类型名.成员` 路径语法，用于精确解析存在同名冲突的类型。
